...
def main():
    ...
    # The menu highlight (8.1kHz, 18,496 bytes/side) and select sound
    # (10.5kHz, 32,096 bytes/side) are long stereo pairs. Rapid input makes
    # private conversions exceed the 2MB pool; shared L/R PCM stays bounded.
    budget = 2 * 1024 * 1024
    for dsp in DSPS:
        highlight = align32((18496 // 2) * dsp // 8100 * 2) + 64
        select = align32((32096 // 2) * dsp // 10500 * 2) + 64
        assert 10 * 2 * highlight > budget
        assert 8 * 2 * select > budget
        assert 2 * (highlight + select) < budget // 2
    print("ok: shared frontend PCM stays bounded under rapid navigation")
