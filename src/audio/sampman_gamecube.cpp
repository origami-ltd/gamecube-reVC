...
cSampleManager::Initialise(void)
{
	...
	// MEM1 audit markers: the user asked what audio costs MEM1. Every MEM1
	// consumer of this backend is bracketed below; the memmap log carries
	// the deltas (bank/ped/talk/channel copies are MEM2 on HW_RVL already).
	extern void gcHeapGuardCheck(const char *marker);
	gcHeapGuardCheck("audio-init-pre");
	...
	gcHeapGuardCheck("audio-banks-done");
	gcLoadTrackLengths();
	gcHeapGuardCheck("audio-init-post");
...
cSampleManager::StartStreamedFile(tTrack nFile, uint32 nPos, uint8 nStream)
{
	...
	{
		extern void gcHeapGuardCheck(const char *marker);
		gcHeapGuardCheck("audio-stream-opened");
	}
