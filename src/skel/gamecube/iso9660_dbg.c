...
{
	// The cache window is a FIXED 16-sector-aligned 32KB block. Unaligned
	// fills made boundary-crossing reads land on ever-shifting windows —
	// under Dolphin's one-late DI delivery that produced deterministic
	// garbage at 32KB-crossing reads (hud.txd +0x28000, neo.txd: the
	// streamer retried the "corrupt" file forever).
	u32 alignedSector = (u32)(offset / SECTOR_SIZE) & ~15u;
	u32 inbuf = (u32)(offset - (u64)alignedSector * SECTOR_SIZE);
	const DISC_INTERFACE *disc = mdescr->disc_interface;
	(void)disc;

	len = MIN(BUFFER_SIZE - inbuf, len);

	if (mdescr->cache_sectors && alignedSector == mdescr->cache_start)
	{
		memcpy(ptr, mdescr->read_buffer + inbuf, len);
		return len;
	}

	{
		dvdcmdblk blk;
		s32 r = -1;
		// Dolphin's DI delivers each transfer one command late (the double
		// issue masks that) and intermittently errors; bounded retries turn
		// a transient error into a success.
		for(int attempt = 0; attempt < 6 && r <= 0; attempt++){
			DVD_ReadPrio(&blk, mdescr->read_buffer,
			    (BUFFER_SIZE / SECTOR_SIZE) << 11, (s64)alignedSector << 11, 2);
			r = DVD_ReadPrio(&blk, mdescr->read_buffer,
			    (BUFFER_SIZE / SECTOR_SIZE) << 11, (s64)alignedSector << 11, 2);
			if(r <= 0)
				usleep(2000);
		}
		if(r <= 0){
			...
			if(said < 3){
				said++;
				printf("iso: read FAILED s=%u after retries\n", alignedSector);
			}
	...
	mdescr->cache_start = alignedSector;
...
static bool read_directories(MOUNT_DESCR *mdescr)
{
	...
	if (volume)
		...
		printf("iso: no volume descriptor\n");
		return false;
	}
	printf("iso: unicode=%d pt=%u len=%u\n", (int)mdescr->iso_unicode,
	    (unsigned)volume->path_table_be, (unsigned)volume->path_table_len_be);
...
bool ISO9660_MountDbg(const char* name, const DISC_INTERFACE *disc_interface)
{
	char *nameCopy;
	...
	{
		printf("iso: AddDevice failed\n");
		free(mdescr);
		free(devops);
		return false;
	}
	printf("iso: mounted ok\n");
	return true;
}


bool ISO9660_UnmountDbg(const char* name)
{
	devoptab_t *devops;
	MOUNT_DESCR *mdescr;
	char devname[11];
...
const char *ISO9660_GetVolumeLabelDbg(const char *name)
{
	MOUNT_DESCR *mdescr;
	char devname[11];
